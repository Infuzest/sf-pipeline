# BUPA Salesforce DevOps — Pro-Code Developer Guide

The BUPA Salesforce DevOps web UI exists for citizen developers. **You don't need it.** The
...
- **Manual deploy**: Actions → "BUPA Salesforce DevOps Deploy" → Run workflow → pick the
  stage branch, give a reason (audit-logged). The environment gate still applies.
- **Rollback**: Actions → "BUPA Salesforce DevOps Rollback" → env + target release sequence +
